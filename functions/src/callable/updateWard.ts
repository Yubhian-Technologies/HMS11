...
import { UpdateWardRequest, branchCollection } from "@hms/shared";
...
export const updateWard = onCall(async (request) => {
  ...
  const { hospitalId, branchId, wardId, building, floor, name } = UpdateWardRequest.parse(request.data);
  assertOwnHospital(caller, hospitalId);

  const db = getFirestore();
  const ref = db.collection(branchCollection(hospitalId, branchId, "wards")).doc(wardId);

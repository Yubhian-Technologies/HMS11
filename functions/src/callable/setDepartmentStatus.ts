...
import { SetDepartmentStatusRequest, hospitalCollection } from "@hms/shared";
import { writeWithAudit } from "@hms/shared-server";
import { requireCallerRole } from "../services/callable-auth";

/** FR-3.2. admin only, own hospital. */
export const setDepartmentStatus = onCall(async (request) => {
  const caller = requireCallerRole(request, ["admin"]);
  const { hospitalId, departmentId, status } = SetDepartmentStatusRequest.parse(request.data);

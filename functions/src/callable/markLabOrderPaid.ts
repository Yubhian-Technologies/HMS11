...
import { MarkLabOrderPaidRequest } from "@hms/shared";
...
export const markLabOrderPaid = onCall(async (request) => {
  ...
  const snap = await db.collection("labOrders").doc(input.labOrderId).get();
  const order = snap.data();
  if (!snap.exists || order?.hospitalId !== input.hospitalId) {

...
import { SetRoomStatusRequest, branchCollection } from "@hms/shared";
...
export const setRoomStatus = onCall(async (request) => {
  ...
  const { hospitalId, branchId, roomId, status } = SetRoomStatusRequest.parse(request.data);
  assertOwnHospital(caller, hospitalId);

  const db = getFirestore();
  const snap = await db.collection(branchCollection(hospitalId, branchId, "rooms")).doc(roomId).get();
  if (!snap.exists) {
